...
def func(t):
    ...
    kick = soft_clipper((1-cos(65536/(t_kick+512)))*0.5 * kick_envelope) * 70
    ...
    hihat = soft_clipper(random()*hihat_envelope) * 30
    ...
    snare = soft_clipper(((1+sin(131072/(t_snare+512)))*0.5 + random())*0.5 * snare_envelope) * 60
    kick = soft_clipper((1+sin(65536/(t_kick+512)))*0.5 * kick_envelope) * 70
    hihat = soft_clipper(random()*hihat_envelope) * 30

    right_master = saws * sidechain + (kick + snare) * (t % 524288 <= 262144) + hihat

    return [left_master*2,right_master*2]
