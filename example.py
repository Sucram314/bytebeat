from bytebeat import ByteBeat

from math import sin,cos,exp
from random import random

def envelope(t):
    return 1 - min((t/2048)**3,1)

def soft_clipper(x,order=10):
    return 2/(1+exp(-order*x)) - 1

def func(t):
    saws = 0

    saws += ((t*2*2**(
            [9,7,5,7,9,7,5,4]
            [(t>>14)%8]/12))%256-128
        )/15

    saws += ((t*2*2**(
            [16,14,12,14,16,14,12,11]
            [(t>>14)%8]/12))%256-128
        )/15

    saws += ((t*4*2**(
            [9,7,5,7,9,7,5,4]
            [(t>>14)%8]/12))%256-128
        )/15

    saws += ((t*4*2**(
        [
            19,11,12,19,11,12,19,11,12,19,11,12,23,11,19,11,
            19,11,12,19,11,12,19,11,12,24,11,12,23,11,21,11,
            19,11,12,19,11,12,19,11,12,19,11,12,17,11,16,11,
            14,11,12,14,11,12,14,11,12,14,11,12,16,11,14,11,
            12,11,12,19,11,12,19,11,12,19,11,12,23,11,19,11,
            19,11,12,19,11,12,19,11,12,24,11,12,23,11,21,11,
            19,11,12,19,11,12,19,11,12,19,11,12,17,11,16,11,
            14,11,12,14,11,12,14,11,12,14,11,12,16,11,14,11
        ][(t>>10)%128]/12))%256-128
    )/10

    if t % 262144 >= 131072:
        saws += ((t*8*2**(
            [19,19,19,19,19,19,17,17,16,16,16,16,14,14,16,16,9,9,9,21,19,19,23,23,16,16,16,16,8,8,14,14]
            [(t>>12)%32]/12))%256-128
        )/8

    kick_times = [0,16384,22528,32768,49152,52224,55296]
    t_kick = t%65536-max(i for i in kick_times if i <= t%65536)
    kick_envelope = envelope(t_kick)
    kick = soft_clipper(cos(65536/(t_kick+512)) * kick_envelope) * 35

    snare_times = [-65536,8192,24576]
    t_snare = t%32768-max(i for i in snare_times if i <= t%32768)
    snare_envelope = envelope(t_snare)
    snare = soft_clipper((cos(131072/(t_snare+512)) + random() - 0.5)*0.5 * snare_envelope) * 30

    t_hihat = t%4096
    hihat_envelope = envelope(t_hihat*4)
    hihat = soft_clipper((random() - 0.5) * hihat_envelope) * 15

    sidechain = (1 - kick_envelope*0.8 - snare_envelope*0.5)

    left_master =  saws * sidechain + (kick + snare) * (t % 524288 <= 262144) + hihat

    saws = 0

    saws += (((t+128)*2*2**(
            [9,7,5,7,9,7,5,4]
            [(t>>14)%8]/12))%256-128
        )/15

    saws += (((t+128)*2*2**(
            [16,14,12,14,16,14,12,11]
            [(t>>14)%8]/12))%256-128
        )/15

    saws += (((t+128)*4*2**(
            [9,7,5,7,9,7,5,4]
            [(t>>14)%8]/12))%256-128
        )/15

    saws += (((t+128)*4*2**(
        [
            19,11,12,19,11,12,19,11,12,19,11,12,23,11,19,11,
            19,11,12,19,11,12,19,11,12,24,11,12,23,11,21,11,
            19,11,12,19,11,12,19,11,12,19,11,12,17,11,16,11,
            14,11,12,14,11,12,14,11,12,14,11,12,16,11,14,11,
            12,11,12,19,11,12,19,11,12,19,11,12,23,11,19,11,
            19,11,12,19,11,12,19,11,12,24,11,12,23,11,21,11,
            19,11,12,19,11,12,19,11,12,19,11,12,17,11,16,11,
            14,11,12,14,11,12,14,11,12,14,11,12,16,11,14,11
        ][(t>>10)%128]/12))%256-128
    )/10

    if t % 262144 >= 131072:
        saws += (((t+128)*8*2**(
            [19,19,19,19,19,19,17,17,16,16,16,16,14,14,16,16,9,9,9,21,19,19,23,23,16,16,16,16,8,8,14,14]
            [(t>>12)%32]/12))%256-128
        )/8

    kick = soft_clipper(sin(65536/(t_kick+512)) * kick_envelope) * 35
    snare = soft_clipper((sin(131072/(t_snare+512)) + random() - 0.5)*0.5 * snare_envelope) * 30
    hihat = soft_clipper((random() - 0.5) * hihat_envelope) * 15

    right_master = saws * sidechain + (kick + snare) * (t % 524288 <= 262144) + hihat

    return [127+left_master*2,127+right_master*2]

bytebeat = ByteBeat(
    func=func,
    sample_rate=8000,
    channels=2,
    width=1024,
)

#bytebeat.save("output.wav",524288)

bytebeat.play()